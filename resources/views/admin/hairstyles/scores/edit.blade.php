        ...
        <div class="flex flex-col md:flex-row items-center justify-between space-y-4 md:space-y-0">
            <div>
                <h1 class="title text-3xl font-bold text-gray-900 dark:text-white flex items-center mb-4">
                    <i class="fas fa-cogs mr-3"></i>{{ __('admin.edit_hairstyle_score') }}
                </h1>
                <p class="text-sm text-gray-600 dark:text-gray-400 mt-2">{{ __('admin.update_existing_score') }}
                </p>
            </div>
        </div>
    </section>
                ...
                <form action="{{ route('admin.hairstyles.score.update', $hairstyle_score->id) }}" method="POST"
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="criterion_id" id="criterion_id"
                            class="block w-full h-12 px-4 rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-white focus:ring-blue-500 focus:border-blue-500 shadow-sm transition">
                            <option value="">{{ __('admin.select_criterion') }}</option>
                    ...
                    <div class="flex justify-end space-x-4 pt-4">
                        ...
                        <button type="submit"
                            ...
                            <i class="fas fa-save mr-2"></i>{{ __('admin.update_score') }}
                        </button>
                    </div>
                </form>
            </div>
